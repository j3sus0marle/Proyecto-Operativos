...
void calcularTiemposSJF(struct Lista* lista) {
    ...
    // Ordenar la lista por burst time
    struct Nodo* i = lista->cabeza;
    while (i != NULL) {
        struct Nodo* j = i->siguiente;
        while (j != NULL) {
            struct Proceso* procesoI = (struct Proceso*)i->dato;
            struct Proceso* procesoJ = (struct Proceso*)j->dato;

            if (procesoI->burstTime > procesoJ->burstTime) {
